...
import com.radarqr.dating.android.utility.AWSKeys
import java.io.File
...
object AmazonUtil {
    ...
    fun getS3Client(context: Context?): AmazonS3Client? {
        if (amzonS3Client == null) {
            amzonS3Client =
                AmazonS3Client(getCredProvider(context), Region.getRegion(AWSKeys.MY_REGION))
    ...
    fun getTransferUtility(context: Context): TransferUtility? {
        if (transferUtility == null) {
            transferUtility =
                TransferUtility.builder().s3Client(getS3Client(context)).context(context).build()
        }
