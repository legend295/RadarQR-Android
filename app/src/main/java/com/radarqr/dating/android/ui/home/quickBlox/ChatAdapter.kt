...
import com.radarqr.dating.android.ui.home.quickBlox.managers.PROPERTY_NOTIFICATION_TYPE
...
import com.radarqr.dating.android.ui.home.quickBlox.ChatFragment.Companion.PROPERTY_FORWARD_USER_NAME
...
//        }
    ...
    private fun onBindViewNotificationHolder(
    ...
    private fun onBindViewMsgRightHolder(
        holder: NewMessageViewHolder,
        chatMessage: QBChatMessage?
    ) {
        chatMessage?.let {
    ...
    private fun fillTextMessageHolder(
        ...
        if (chatDialog.type == QBDialogType.PRIVATE) {
            fillAvatarHolder(holder, chatMessage, isIncomingMessage)
        } else {
            holder.avatarContainer?.visibility = View.GONE
        }

        chatMessage.handleMessageSentStatus(isIncomingMessage, holder)
    }

    private fun onBindViewAttachRightHolder(
        holder: NewMessageViewHolder,
        chatMessage: QBChatMessage?,
        position: Int
    ) {
    ...
    private fun onBindViewAttachLeftHolder(
        holder: NewMessageViewHolder,
        chatMessage: QBChatMessage?,
        position: Int
    ) {
        chatMessage?.let {
            ...
            if (chatDialog.type != QBDialogType.PRIVATE) {
                fillAvatarHolder(holder, chatMessage, true)
            } else {
                holder.avatarContainer?.visibility = View.GONE
            }
    ...
    private fun fillAvatarHolder(
        holder: NewMessageViewHolder,
        chatMessage: QBChatMessage,
        isIncomingMessage: Boolean
    ) {
        val data = chatDialog.customData.get(USER_DATA) as MatchedData?
        data?.let {
...
//        val user = QbUsersHolder.getUserById(chatMessage.senderId)
            var url: String
            runBlocking {
                url = preferencesHelper.getValue(PreferencesHelper.PreferencesKeys.KEY_IMAGE_URL)
                    .first() ?: ""
            }
            if (data.name.isNotEmpty() && data.profile_pic.isEmpty()) {
                val avatarTitle = data.name[0].toString().uppercase(Locale.getDefault())
                holder.avatarTitle?.text = avatarTitle
            }
            holder.avatarImage?.loadImage(if (isIncomingMessage) data.profile_pic else url)
        }
    }
...
//        setItemAttachClickListener(getAttachListenerByType(position), holder, getAttachment(position), position)
    ...
    private fun QBChatMessage.handleMessageSentStatus(
        isIncomingMessage: Boolean,
        holder: NewMessageViewHolder
    ) {
        if (!isIncomingMessage) {
            val read = isRead(this)
            val delivered = isDelivered(this)
            if (read) {
                holder.ivMessageStatus?.setImageDrawable(
            ...
            when {
                photo || image -> {
                    ...
                    val forwardedFromName =
                        chatMessage.getProperty(PROPERTY_FORWARD_USER_NAME) as String?
                    if (forwardedFromName != null) {
...
//                            .into(holder.ivImageAttachPreview)
                ...
                file -> {
                    ...
                    holder.tvFileSize?.text = android.text.format.Formatter.formatShortFileSize(
                        context,
                        attachment.size.toLong()
                    )
...
//    }
    ...
    private fun isReadByCurrentUser(chatMessage: QBChatMessage): Boolean {
        return !CollectionsUtil.isEmpty(chatMessage.readIds) && chatMessage.readIds.contains(
            quickBloxManager.qbSessionManager.sessionParameters.userId
        )
    }
    ...
    private fun isRead(chatMessage: QBChatMessage): Boolean {
        ...
        val currentUserId = quickBloxManager.qbSessionManager.sessionParameters.userId
    ...
    override fun onBindHeaderViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        chatMessage?.let {
            ...
            val lastYearFormat = SimpleDateFormat("MMM dd, yyyy", Locale.ENGLISH)
...
//        dateTextView.layoutParams = layoutParams
    ...
    private fun isIncoming(chatMessage: QBChatMessage): Boolean {
        return chatMessage.senderId != null && chatMessage.senderId != quickBloxManager.qbSessionManager.sessionParameters.userId
    }
...
//    }
    ...
    inner class NewMessageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var avatarImage: ShapeableImageView? = itemView.findViewById(R.id.ivAvatar)

        var llMessageContainer: LinearLayout? = itemView.findViewById(R.id.ll_message_container)
        var llMessageBodyContainer: LinearLayout? =
            itemView.findViewById(R.id.ll_message_body_container)
        var llMessageBodyForwardContainer: LinearLayout? =
            itemView.findViewById(R.id.ll_forwarded_container)
        ...
        var rlImageAttachmentContainer: RelativeLayout? =
            itemView.findViewById(R.id.rl_image_attach_container)
        var ivImageAttachPreview: ImageView? = itemView.findViewById(R.id.iv_attach_image_preview)
        var llImageForwardContainer: LinearLayout? =
            itemView.findViewById(R.id.ll_image_forwarded_container)
        var tvImageForwardedFromUser: TextView? =
            itemView.findViewById(R.id.tv_image_forward_from_user)
        var pbImageProgress: ProgressBar? = itemView.findViewById(R.id.pb_attach_image)

        var rlVideoAttachmentContainer: RelativeLayout? =
            itemView.findViewById(R.id.rl_video_attach_container)
        var llVideoForwardContainer: LinearLayout? =
            itemView.findViewById(R.id.ll_video_forwarded_container)
        var llVideoForwardedFromUser: TextView? =
            itemView.findViewById(R.id.tv_video_forward_from_user)
        ...
        var rlFileAttachmentContainer: RelativeLayout? =
            itemView.findViewById(R.id.rl_file_attach_container)
        var llFileForwardContainer: LinearLayout? =
            itemView.findViewById(R.id.ll_file_forwarded_container)
        var llFileForwardedFromUser: TextView? =
            itemView.findViewById(R.id.tv_file_forward_from_user)
...
//                        attachClickListener.onAttachmentClicked(itemViewType, it, attachment)
    ...
    private inner class LoaderAsyncTask internal constructor(
