package com.radarqr.dating.android.utility.seekbar
...
import com.radarqr.dating.android.utility.seekbar.utils.BitmapUtil
import com.radarqr.dating.android.utility.seekbar.utils.PixelUtil
...
class RangeSeekBar @JvmOverloads constructor(
    ...
    var oldTouchValue = 0.0F
    var minPos = 0.0F
    var differenceLeft = 0.0F
    var differenceRight = 0.0F
    private var endTextDifference = 0.0F
    ...
    private var textFont = ResourcesCompat.getFont(getContext(), R.font.hkgrotesk_semibold)
    ...
    private fun init() {
        context.resources.apply {
            BitmapUtil.apply {
                thumbImage =
                    toBitmap(
                        ContextCompat.getDrawable(context, R.drawable.green_round_thumb)
                            ?: getDrawable(R.drawable.green_round_thumb)
                    )
                thumbPressedImage = toBitmap(
                    ContextCompat.getDrawable(context, R.drawable.green_round_thumb)
                        ?: getDrawable(R.drawable.green_round_thumb)
                )
                thumbDisabledImage = toBitmap(
                    ContextCompat.getDrawable(context, R.drawable.green_round_thumb)
                        ?: getDrawable(R.drawable.green_round_thumb)
                )
            }
        ...
        attrs?.let {
            context.obtainStyledAttributes(it, R.styleable.RangeSeekBar).apply {
                getDrawable(R.styleable.RangeSeekBar_thumbsNormal)?.let { drawable ->
                    thumbImage = BitmapUtil.toBitmap(drawable)
                }
                getDrawable(R.styleable.RangeSeekBar_thumbsDisabled)?.let { drawable ->
                    thumbDisabledImage = BitmapUtil.toBitmap(drawable)
                }
                getDrawable(R.styleable.RangeSeekBar_thumbsPressed)?.let { drawable ->
                    thumbPressedImage = BitmapUtil.toBitmap(drawable)
                }
                ...
                transitionBarColor =
                    getColor(R.styleable.RangeSeekBar_transitionColor, transitionBarColor)
                ...
                thumbTextPosition =
                    getInt(R.styleable.RangeSeekBar_showThumbsText, thumbTextPosition)
                additionalTextPosition =
                    getInt(R.styleable.RangeSeekBar_showAdditionalText, additionalTextPosition)
                textSize = getDimensionPixelSize(
                    R.styleable.RangeSeekBar_textSize,
                    textSize
                ) //todo maybe problems with text size here
                ...
                type = getInt(R.styleable.RangeSeekBar_type, type)
                ...
                if (!stepValueValidation(
                        minValue,
                        maxValue,
                        stepValue
                    )
                ) throw Exception("Incorrect min/max/step, it must be: (maxValue - minValue) % stepValue == 0f")
                valueType = getInt(R.styleable.RangeSeekBar_valueType, valueType)
                stepsCount = ((maxValue - minValue) / stepValue).toInt()

                isRoundedCorners =
                    getBoolean(R.styleable.RangeSeekBar_roundedCorners, isRoundedCorners)

                barHeight = getDimensionPixelSize(R.styleable.RangeSeekBar_barHeight, barHeight)

                additionalTextMargin = getDimensionPixelSize(
                    R.styleable.RangeSeekBar_additionalTextMargin,
                    additionalTextMargin
                )
                thumbTextMargin = getDimensionPixelSize(
                    R.styleable.RangeSeekBar_thumbsTextMargin,
                    thumbTextMargin
                )
            ...
            event?.let { event ->
                ...
                when (event.action and MotionEvent.ACTION_MASK) {
                    ...
                    MotionEvent.ACTION_MOVE -> {
                        pressedThumb?.let {
                            listenerRealTime?.let {
                                when (valueType) {
                                    INT -> it.onValuesChanging(
                                        getSelectedMinValue().toInt(),
                                        getSelectedMaxValue().toInt()
                                    )
                                    FLOAT -> it.onValuesChanging(
                                        getSelectedMinValue().toFloat(),
                                        getSelectedMaxValue().toFloat()
                                    )
                    ...
                    MotionEvent.ACTION_UP -> {
                        ...
                        listenerPost?.let {
                            when (valueType) {
                                INT -> it.onValuesChanged(
                                    getSelectedMinValue().toInt(),
                                    getSelectedMaxValue().toInt()
                                )
                                FLOAT -> it.onValuesChanged(
                                    getSelectedMinValue().toFloat(),
                                    getSelectedMaxValue().toFloat()
                                )
                                else -> throw Exception("Unknown value type")
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.apply {
            ...
            paint.textSize = textSize.toFloat()
            paint.color =
                textColor                 //todo in future change textColor thumb values and left/center/right text
            ...
            var maxPosition = min(
                width - (maxTextWidth + endTextDifference),
            ...
            val overlap = (minPosition + 30) + minTextWidth - (maxPosition) + spacing
            if (overlap > 0f) {
                minPosition -= (overlap * normalizedMinValue / (normalizedMinValue + 1 - normalizedMaxValue + 0.1)).toFloat()
                maxPosition += (overlap * (1 - normalizedMaxValue) / (normalizedMinValue + 1 - normalizedMaxValue - 0.35)).toFloat()
            }
            ...
            if (thumbTextPosition != THUMB_TEXT_POSITION_NONE) {
                ...
                drawText(
                    if (type == 1) convertToFeetInchesHeight(minText) else minText,
                    minPosition,
                    yPosition,
                    paint
                )
                ...
                drawText(
                    if (type == 1) convertToFeetInchesHeight(maxText) else maxText,
                    maxPosition,
                    yPosition,
                    paint
                )
            ...
            val centerTextPosition =
                (maxPosition + minPosition) * 0.5f - centerTextWidth * 0.5f + spacing * 4
            var textPosition = centerTextPosition

            if (centerTextPosition < leftTextWidth + spacing) textPosition = leftTextWidth + spacing
            if (centerTextPosition + centerTextWidth > width - rightTextWidth - padding + spacing * 4) textPosition =
                width - rightTextWidth - padding - centerTextWidth + spacing * 4
    ...
    private fun convertToFeetInchesHeight(str: String): String {
        ...
        val inches = /*Math.round(value / 2.54 - feet * 12).toInt()*/ (0.3937 * value - feet * 12).toInt()
    ...
    private fun removeRedundantNumberPart(number: String) =
        when (valueType) {
            ...
            FLOAT -> stepValue.toString().apply {
                return when {
                    contains(".") -> {
                        val numbersCountDifference =
                            (length - indexOf(".") - 1) - (number.length - number.indexOf(".") - 1)
                        when {
                            ...
                            (numbersCountDifference) < 0 -> number.substring(
                                0,
                                number.indexOf(".") + (length - indexOf("."))
                            )
                            else -> number
                        }
                    }
    ...
    private fun drawThumb(screenCoord: Float, pressed: Boolean, canvas: Canvas) {
        ...
        canvas.drawBitmap(
            ...
            center.y.toFloat() - if (pressed) getThumbPressedHalfHeight() else getThumbHalfHeight(),
            paint
        )
    }
    ...
    private fun isInThumbRange(touchX: Float, normalizedThumbValue: Double) =
        abs(touchX - normalizedToScreen(normalizedThumbValue)) <= getThumbHalfWidth()

    private fun normalizedToScreen(normalizedPos: Double) =
        (padding + normalizedPos * (width - 2 * padding)).toFloat()
    ...
    private fun setNormalizedMinValue(value: Double) {
        normalizedMinValue = max(0.0, min(1.0, min(value, normalizedMaxValue - differenceLeft)))
        invalidate()
    }

    private fun setNormalizedMaxValue(value: Double) {
        normalizedMaxValue = max(0.0, min(1.0, max(value, normalizedMinValue + differenceRight)))
        invalidate()
    }
    ...
    private fun valueToNormalize(value: Double) =
        ((maxValue - minValue) * (value * 100)) / 100 + minValue

    private fun normalizeToValue(value: Float) =
        (((value - minValue) * 100) / (maxValue - minValue)) / 100

    private fun getSelectedMinValue() =
        getValueAccordingToStep(valueToNormalize(normalizedMinValue))

    private fun getSelectedMaxValue() =
        getValueAccordingToStep(valueToNormalize(normalizedMaxValue))

    private fun getValueAccordingToStep(value: Double) =
        ((value.roundToInt().toBigDecimal() / stepValue.toBigDecimal()).toInt()).toBigDecimal() * stepValue.toBigDecimal()
    ...
    fun setCurrentValues(leftValue: Float, rightValue: Float) {
        ...
        if (!stepValueValidation(leftValue, maxValue, stepValue) || !stepValueValidation(
                minValue,
                rightValue,
                stepValue
            )
        ) throw Exception("You can't set these values according to your step")
    ...
    fun setCurrentValues(leftValue: Int, rightValue: Int) {
        if (leftValue > rightValue) throw Exception("LeftValue can't be higher than rightValue")
        if (leftValue < minValue || rightValue > maxValue) throw Exception("Out of range")
        if (!stepValueValidation(leftValue.toFloat(), maxValue, stepValue) || !stepValueValidation(
                minValue,
                rightValue.toFloat(),
                stepValue
            )
    ...
    fun setDifference(leftValue: Float, rightValue: Float) {
        this.differenceLeft = leftValue
        this.differenceRight = rightValue
        invalidate()
    }

    fun setRightTextDifference(value: Float) {
        this.endTextDifference = value
        invalidate()
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val heightThumbText = if (thumbTextPosition == THUMB_TEXT_POSITION_NONE ||
            thumbTextPosition == THUMB_TEXT_POSITION_CENTER
        ) 0 else PixelUtil.dpToPx(context, thumbTextMargin) + PixelUtil.dpToPx(
            context,
            textSize
        ) / 2

        val heightAdditionalText = if (additionalTextPosition == ADDITIONAL_TEXT_POSITION_NONE ||
            additionalTextPosition == ADDITIONAL_TEXT_POSITION_CENTER
        ) 0 else PixelUtil.dpToPx(context, additionalTextMargin) + PixelUtil.dpToPx(
            context,
            textSize
        ) / 2
