...
object ChatHelper {
    ...
    fun deleteDialog(qbDialog: QBChatDialog, callback: QBEntityCallback<Void>) {
        if (qbDialog.type == QBDialogType.PUBLIC_GROUP) {
            shortToast("Group chat cannot be deleted")
        } else {
            QBRestChatService.deleteDialog(qbDialog.dialogId, false)
                .performAsync(QbEntityCallbackWrapper(callback))
        }
    }
    ...
    fun updateDialogUsers(
        ...
        QBRestChatService.updateGroupChatDialog(qbDialog, qbRequestBuilder)
            .performAsync(object : QbEntityCallbackWrapper<QBChatDialog>(callback) {
    ...
    /* fun getDialogsOnline(requestBuilder: QBRequestGetBuilder, callback: QBEntityCallback<ArrayList<QBChatDialog>>) {

         QBRestChatService.getChatDialogs(null, requestBuilder).performAsync(
                 object : QbEntityCallbackWrapper<ArrayList<QBChatDialog>>(callback) {
                     override fun onSuccess(dialogs: ArrayList<QBChatDialog>, bundle: Bundle?) {
                         getUsersFromDialogsOnline(dialogs, callback)
    ...
    fun getDialogsOnline(
        ...
        QBRestChatService.getChatDialogs(null, requestBuilder).performAsync(
            object : QbEntityCallbackWrapper<ArrayList<QBChatDialog>>(callback) {
                override fun onSuccess(t: ArrayList<QBChatDialog>, bundle: Bundle?) {
                    callback.onSuccess(t, bundle)
    ...
    fun getCustomDialogsOnline(
        dialogs: ArrayList<QBChatDialog>,
        requestBuilder: QBRequestGetBuilder,
        callback: QBEntityCallback<ArrayList<QBCustomObject>>
    ) {

        QBCustomObjects.getObjects("occupantDetails", requestBuilder)
            .performAsync(object : QBEntityCallback<ArrayList<QBCustomObject>> {
    ...
    fun getDialogsInPerson(
        ...
        QBRestChatService.getChatDialogs(null, requestBuilder).performAsync(
            object : QbEntityCallbackWrapper<ArrayList<QBChatDialog>>(callback) {
                override fun onSuccess(dialogs: ArrayList<QBChatDialog>, bundle: Bundle?) {
                    getUsersFromDialogsInPerson(dialogs, callback)
                    // Not calling callback.onSuccess(...) because
                    // we want to load chat userList before triggering callback
                }
            })
                ...
                if (qbUsers != null) {
                    ...
                    bundle?.let {
                        ...
                        if (totalPages > currentPage) {
                            ...
                            loadUsersByIDsFromDialog(userIDs, requestBuilder, callback)
//                            loadUsersByFilterFromDialog(userIDs, requestBuilder, callback)
                        } else {
    ...
    private fun loadUsersByFilterFromDialog(
        userIDs: Collection<Int>,
        requestBuilder: QBPagedRequestBuilder,
        callback: QBEntityCallback<ArrayList<QBUser>>
    ) {
        /*val valueList: HashMap<String, String> = HashMap()
        valueList["class_name"] = "occupantDetails"
        QBUsers.getUsersByFilter(valueList.values, "all", requestBuilder).performAsync(object : QBEntityCallback<ArrayList<QBUser>> {
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, bundle: Bundle?) {
                if (qbUsers != null) {
                    usersLoadedFromDialog.addAll(qbUsers)
                    QbUsersHolder.putUsers(qbUsers)
...
//                            loadUsersByIDsFromDialog(userIDs, requestBuilder, callback)
        ...
        QBCustomObjects.getObjects("occupantDetails")
*/
    }
    ...
    private fun getUsersFromDialogsOnline(
        dialogs: ArrayList<QBChatDialog>,
        callback: QBEntityCallback<ArrayList<QBChatDialog>>
    ) {
        val userIds = HashSet<Int>()
        ...
        val requestBuilder = QBPagedRequestBuilder(USERS_PER_PAGE, 1)
        usersLoadedFromDialogs.clear()
        /* loadUsersByIDsFromDialogsOnline(userIds, requestBuilder, object : QBEntityCallback<ArrayList<QBUser>> {
             override fun onSuccess(qbUsers: ArrayList<QBUser>?, b: Bundle?) {
                 callback.onSuccess(dialogs, b)
             }
    ...
    private fun loadUsersByIDsFromDialogsOnline(
        userIDs: Collection<Int>,
        requestBuilder: QBPagedRequestBuilder,
        callback: QBEntityCallback<ArrayList<QBUser>>
    ) {
            ...
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, bundle: Bundle?) {
                if (qbUsers != null) {
                    ...
                    bundle?.let {
                        ...
                        if (totalPages > currentPage) {
                            requestBuilder.page = currentPage + 1
                            loadUsersByIDsFromDialogsOnline(userIDs, requestBuilder, callback)
                        } else {
                            callback.onSuccess(usersLoadedFromDialogs, bundle)
                        }
                    }
    ...
    private fun getUsersFromDialogsInPerson(
        dialogs: ArrayList<QBChatDialog>,
        callback: QBEntityCallback<ArrayList<QBChatDialog>>
    ) {
        val userIds = HashSet<Int>()
        ...
        val requestBuilder = QBPagedRequestBuilder(USERS_PER_PAGE, 1)
        usersLoadedFromDialogs.clear()
        loadUsersByIDsFromDialogsInPerson(
            userIds,
            requestBuilder,
            object : QBEntityCallback<ArrayList<QBUser>> {
                override fun onSuccess(qbUsers: ArrayList<QBUser>?, b: Bundle?) {
    ...
    private fun loadUsersByIDsFromDialogsInPerson(
        userIDs: Collection<Int>,
        requestBuilder: QBPagedRequestBuilder,
        callback: QBEntityCallback<ArrayList<QBUser>>
    ) {
            ...
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, bundle: Bundle?) {
                if (qbUsers != null) {
                    ...
                    bundle?.let {
                        ...
                        if (totalPages > currentPage) {
                            requestBuilder.page = currentPage + 1
                            loadUsersByIDsFromDialogsInPerson(userIDs, requestBuilder, callback)
                        } else {
                            callback.onSuccess(usersLoadedFromDialogs, bundle)
                        }
                    }
    ...
    private fun loadUsersByTagsFromDialogs(
        userIDs: Collection<Int>,
        requestBuilder: QBPagedRequestBuilder,
        callback: QBEntityCallback<ArrayList<QBUser>>
    ) {
        ...
        QBUsers.getUsersByTags(userId, requestBuilder).performAsync(object : QBEntityCallback<ArrayList<QBUser>> {
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, bundle: Bundle?) {
                if (qbUsers != null) {
                    usersLoadedFromDialogs.addAll(qbUsers)
                    QbUsersHolder.putUsers(qbUsers)
                    bundle?.let {
                        ...
                        if (totalPages > currentPage) {
                            requestBuilder.page = currentPage + 1
                            loadUsersByTagsFromDialogs(userIDs, requestBuilder, callback)
                        } else {
                            callback.onSuccess(usersLoadedFromDialogs, bundle)
                        }
                    }
    ...
    fun getCurrentUser(): QBUser? {
        return if (SharedPrefsHelper.hasQbUser()) {
